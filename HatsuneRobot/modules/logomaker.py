...
from HatsuneRobot import telethn as tbot, ubot2
...
async def logo_gen(event):
    ...
    if event.reply_to_msg_id:
        ...
        async for i in ubot2.iter_messages(
            "@hatsunelogo", filter=InputMessagesFilterPhotos
        ):
            pics.append(i)
        id_ = random.choice(pics)
        bg_ = await id_.download_media()
        fpath_ = glob.glob("./HatsuneRobot/resources/fonts/*")
    ...
    if os.path.exists(font_):
        if not font_.startswith("./HatsuneRobot/resources/fonts"):
            os.remove(font_)
